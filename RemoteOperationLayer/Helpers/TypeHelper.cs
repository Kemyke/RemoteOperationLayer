...
namespace ArdinHelpers
{
    public static class TypeHelper
    {
        ...
        private static Dictionary<string, object[]> _localCacheGetTypeCustomAttributes = new Dictionary<string, object[]>();
        private static Dictionary<string, object[]> localCacheGetTypeCustomAttributes
        {
            get
            {
                return _localCacheGetTypeCustomAttributes;
            }
        }
        ...
        private static Dictionary<string, string> _localCacheGetShortAssemblyQualifiedTypeName = new Dictionary<string, string>();
        private static Dictionary<string, string> localCacheGetShortAssemblyQualifiedTypeName
        {
            get
            {
                return _localCacheGetShortAssemblyQualifiedTypeName;
            }
        }

        private static Dictionary<string, object[]> _localCacheHasCustomAttributes = new Dictionary<string, object[]>();
        private static Dictionary<string, object[]> localCacheHasCustomAttributes
        {
            get
            {
                return _localCacheHasCustomAttributes;
            }
        }

        private static Dictionary<string, Type> _localCacheClassStrongName = new Dictionary<string, Type>();
        private static Dictionary<string, Type> localCacheClassStrongName
        {
            get
            {
                return _localCacheClassStrongName;
            }
        }
        ...
        public static string GetShortAssemblyQualifiedTypeName(Type type)
        {
            if (type == null)
            {
                throw new ArgumentNullException("type");
            ...
            if (!localCacheGetShortAssemblyQualifiedTypeName.TryGetValue(cacheKey, out ret))
            {
                AssemblyName an = type.Assembly.GetName();
                ret = String.Concat(type.FullName, ", ", an.Name);

                // check whether type name + assembly name is enough (e.g. for DataTable it is not!)
                Type typeFound = Type.GetType(ret);
                if (type != typeFound)
                {
                    // add version too
                    ret = String.Concat(ret, ", Version=", an.Version.ToString());

                    // check whether type name + assembly name + version is enough
                    typeFound = Type.GetType(ret);
                    if (type != typeFound)
                    {
                        // fallback to AssemblyQualifiedName
                        ret = type.AssemblyQualifiedName;
                    }
                }

                localCacheGetShortAssemblyQualifiedTypeName.Add(cacheKey, ret);
            }

            return ret;
        }

        public static IEnumerable<Type> GetTypes(Assembly a)
        {
            ...
            try
            {
                typeList = a.GetTypes().ToList();
            }
            catch (ReflectionTypeLoadException ex)
            {
                System.Diagnostics.Debug.WriteLine(ex.ToString());
                typeList = ex.Types.Where(i => i != null).ToList();
            }
        ...
        public static bool HasCustomAttribute(PropertyInfo pi, Type customAttributeType)
        {
            if (pi == null)
            {
            ...
            string cacheKey = String.Concat(pi.DeclaringType.FullName, ":", pi.Name, ":", customAttributeType.FullName);
            object[] customAttributes;
            if (!localCacheHasCustomAttributes.TryGetValue(cacheKey, out customAttributes))
            {
                customAttributes = pi.GetCustomAttributes(customAttributeType, false);
                localCacheHasCustomAttributes.Add(cacheKey, customAttributes);
            }

            ret = (customAttributes.Length != 0);
        ...
        public static bool HasCustomAttribute(Type t, Type customAttributeType)
        {
            ...
            object[] customAttributes = GetCustomAttributes(t, customAttributeType);
            ret = (customAttributes.Length != 0);

            return ret.Value;
        }

        public static object[] GetCustomAttributes(Type t, Type customAttributeType)
        {
            if (t == null)
            {
                throw new ArgumentNullException("t");
            ...
            string cacheKey = String.Concat(t.FullName, ":", customAttributeType.FullName);
            if (!localCacheGetTypeCustomAttributes.TryGetValue(cacheKey, out ret))
            {
                ret = t.GetCustomAttributes(customAttributeType, false);
                localCacheGetTypeCustomAttributes.Add(cacheKey, ret);
            }

            return ret;
        }

        public static object[] GetCustomAttributes(MethodInfo mi, Type customAttributeType)
        {
            if (mi == null)
            {
            ...
            string cacheKey = String.Concat(mi.DeclaringType.FullName, ":", mi.Name, ":", customAttributeType.FullName);
            if (!localCacheGetMethodInfoCustomAttributes.TryGetValue(cacheKey, out ret))
            {
                ret = mi.GetCustomAttributes(customAttributeType, false);
                localCacheGetMethodInfoCustomAttributes.Add(cacheKey, ret);
            }
        ...
        public static Type GetType(string typeName, Func<AssemblyName, Assembly> assemblyResolver, Func<Assembly, string, bool, Type> typeResolver)
        {
            ...
            if (!localCacheClassStrongName.TryGetValue(typeName, out ret))
            {
                ret = Type.GetType(typeName, assemblyResolver, typeResolver);
                localCacheClassStrongName.Add(typeName, ret);
            }
            return ret;
        }
